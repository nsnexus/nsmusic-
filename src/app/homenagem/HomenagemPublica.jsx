...
function HomenagemContent() {
  ...
  const formatAudioUrl = (url) => {
    if (!url) return '';
    if (typeof url === 'string' && url.includes('audiopipe.suno.ai')) {
      const match = url.match(/item_id=([a-f0-9\-]+)/i);
      if (match && match[1]) {
  ...
  const primaryAudioUrl = formatAudioUrl(order.audioUrl || (order.audioFiles && order.audioFiles[0]) || '');
  const secondAudioUrl = formatAudioUrl(order.audioFiles && order.audioFiles[1] ? order.audioFiles[1] : '');
