 ...
 *
 * Migração parcial iniciada em 2026-08-07 depois de um bloqueio de conta na W-API (provedor não
 * oficial) — só a mensagem de "música pronta" (maior volume, disparada pra todo cliente que gera
 * uma prévia) sai por aqui por enquanto. As demais mensagens continuam na W-API acima. Diferente da
 * W-API, aqui só dá pra mandar texto livre dentro de uma janela de 24h depois do cliente escrever
 * pra gente — mensagem iniciada pela empresa (é o nosso caso, sempre) precisa de um Template
 * pré-aprovado pela Meta. Por isso não existe um "sendWhatsAppMessage" genérico aqui: cada tipo de
 * mensagem tem sua própria função com as variáveis do Template já aprovado.
 */
...
export const sendMusicReadyTemplate = async (phone, { customerName, honoreeName, deliveryUrl }, env = {}) => {
  ...
  const numbersToSend = [formattedNumber];
  // Mesmo ajuste da W-API: para celular brasileiro com 13 dígitos, tenta também a variante de 12
  // dígitos sem o 9º dígito inicial — números cadastrados no WhatsApp de formas diferentes.
  if (formattedNumber.startsWith('55') && formattedNumber.length === 13 && formattedNumber[4] === '9') {
    const withoutNine = `${formattedNumber.substring(0, 4)}${formattedNumber.substring(5)}`;
    numbersToSend.push(withoutNine);
  }
